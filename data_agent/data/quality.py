...
class DataQualityAssessor:
    ...
    def _assess_completeness(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        completeness_score = ((total_cells - missing_cells) / total_cells * 100).round(2)
        ...
        return {
            ...
            "missing_percentage_by_column": missing_pct_by_column[missing_pct_by_column > 0].to_dict(),
            "columns_mostly_missing": missing_pct_by_column[missing_pct_by_column > 80].index.tolist()
        }
    ...
    def _assess_consistency(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in df.select_dtypes(include=['object']).columns:
            ...
            if len(sample) > 0:
                ...
                inconsistent_cases = {k: list(v) for k, v in case_variants.items() if len(v) > 1}
                if inconsistent_cases and len(inconsistent_cases) < len(case_variants) * 0.5:
                    issues.append({
                        "column": col,
                ...
                if df[col].dtype in [np.number]:
                    ...
                    if len(invalid_ages) > 0:
                        validity_issues.append({
                            ...
                            "description": f"Invalid age values (< 0 or > 150)"
                        })
            
            # Date columns shouldn't have future dates if they represent past events
            if 'datetime' in str(df[col].dtype):
                future_dates = df[df[col] > pd.Timestamp.now()]
                if len(future_dates) > 0 and 'created' in col_lower or 'born' in col_lower:
                    validity_issues.append({
                        "column": col,
                        "type": "future_dates",
                        "count": len(future_dates),
                        "description": f"Future dates in historical data"
                    })
            ...
            if any(keyword in col_lower for keyword in ['id', 'key', 'index', 'uuid']):
                ...
                if unique_count < total_count:
                    id_column_issues.append({
                        ...
                        "description": f"ID column has non-unique values"
                    })
    ...
    def _assess_accuracy(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in df.select_dtypes(include=['object']).columns:
            ...
            if len(sample) > 0:
                ...
                placeholder_values = ['unknown', 'null', 'n/a', 'na', 'none', '', 'missing']
                ...
                if placeholder_count > len(sample) * 0.1:  # More than 10%
                    accuracy_issues.append({
                        ...
                        "percentage": round(placeholder_count / len(sample) * 100, 2),
                        "description": f"High number of placeholder values"
                    })
        ...
        if len(numeric_cols) > 1:
            ...
            if perfect_correlations:
                accuracy_issues.append({
                    ...
                    "description": f"Suspiciously perfect correlations detected"
                })
    ...
    def _generate_recommendations(self, assessment: Dict[str, Any]) -> List[str]:
        ...
        if completeness["columns_mostly_missing"]:
            recommendations.append(f"Consider removing columns with >80% missing data: {completeness['columns_mostly_missing']}")
        
        # Uniqueness recommendations
        uniqueness = assessment["uniqueness"]
        if uniqueness["duplicate_rows"] > 0:
            recommendations.append(f"Remove {uniqueness['duplicate_rows']} duplicate rows")
        
        if uniqueness["duplicate_columns"]:
            recommendations.append(f"Consider removing duplicate columns: {uniqueness['duplicate_columns']}")
        ...
        for issue in assessment["issues"]:
            if issue["severity"] == "critical":
                if issue["type"] == "mostly_missing":
                    recommendations.append(f"Remove or impute column '{issue['column']}'")
                elif issue["type"] == "constant_column":
                    recommendations.append(f"Remove constant column '{issue['column']}'")
...
class DataPreprocessor:
    ...
    def clean_dataset(self, df: pd.DataFrame, options: Dict[str, Any] = None) -> pd.DataFrame:
        """
        ...
        if missing_threshold < 1.0:
            ...
            if len(cols_to_drop) > 0:
                ...
                logger.info(f"Removed columns with >{missing_threshold*100}% missing: {list(cols_to_drop)}")

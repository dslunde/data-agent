...
class DatasetDownloader:
    ...
    def _try_download_method(self, session: requests.Session, method_name: str, url: str, headers: dict = None) -> requests.Response:
    ...
    def download_from_google_drive(
        ...
        try:
            ...
            if not response:
                ...
                initial_response = session.get(f"https://drive.google.com/uc?export=download&id={file_id}")
                
                if initial_response.status_code == 200 and 'text/html' in initial_response.headers.get('content-type', ''):
                    ...
                    for pattern in token_patterns:
                        ...
                        if match:
                            token = match.group(1)
                            logger.info(f"Found token with pattern {pattern}: {token[:10]}...")
                            
                            confirmed_url = f"https://drive.google.com/uc?export=download&id={file_id}&confirm={token}"
                            response = self._try_download_method(session, "token_confirmed", confirmed_url)
                            if response:
                                break
            
            if not response:
                raise ValueError("All download methods failed - unable to bypass Google Drive restrictions")
            ...
            if actual_size < 1000:
                ...
                try:
                    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                        ...
                        if any(tag in first_content.lower() for tag in ['<html', '<!doctype', '<head', '<body']):
                            logger.error("Downloaded file appears to be HTML, not the expected file")
                            file_path.unlink()
                            raise ValueError("Downloaded HTML page instead of file - the file may be too large for direct download or requires different permissions")
                except:
                    pass  # If we can't read as text, it's probably binary (good)
            
            logger.info(f"Successfully downloaded dataset to {file_path} ({actual_size:,} bytes)")
            return file_path
    ...
    def download_dataset(
        ...
        if "drive.google.com" in url:
            try:
                ...
                self._show_manual_download_instructions(url, filename or "dataset.parquet")
                raise
        else:

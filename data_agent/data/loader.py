...
class SchemaInfo:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "missing_percentages": {
                k: round(v, 2) for k, v in self.missing_percentages.items()
            },
...
class DataLoader:
    ...
    def _load_parquet(
        self, file_path: Path, sample_size: Optional[int]
    ) -> pd.DataFrame:
        """Load parquet file."""
        if sample_size:
    ...
    def _load_csv(self, file_path: Path, sample_size: Optional[int]) -> pd.DataFrame:
        ...
        df = pd.read_csv(
            file_path, nrows=sample_size, parse_dates=datetime_cols, low_memory=False
        )

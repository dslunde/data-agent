...
class PatternAnalyzer:
        ...
        try:
            ...
            result = {
                ...
                "summary": {
                    ...
                    "average_correlation": float(np.abs(corr_matrix.values[np.triu_indices_from(corr_matrix.values, k=1)]).mean())
                }
            }
    ...
    def time_series_patterns(
        ...
        try:
            ...
            for col in value_columns:
                col_patterns = self._analyze_time_series_column(df_clean, date_column, col, detect_seasonality)
            ...
            result = {
                ...
                "date_range": {
                    ...
                    "span_days": (df_clean[date_column].max() - df_clean[date_column].min()).days
                },
                "patterns": patterns
            }
        ...
        try:
            ...
            for i, col1 in enumerate(categorical_columns):
                for col2 in categorical_columns[i+1:]:
                    association_info = self._calculate_association(df, col1, col2, min_support)
                    if association_info:
    ...
    def _kmeans_clustering(
        ...
        if n_clusters is None:
            n_clusters = self._find_optimal_clusters(scaled_data, max_k=min(10, len(data) // 2))
    ...
    def _analyze_time_series_column(
        ...
        patterns = {
            ...
            "range": {
                ...
                "span": float(clean_data[value_col].max() - clean_data[value_col].min())
            }
        }
        
        # Detect seasonality if requested
        if detect_seasonality and len(clean_data) >= 24:  # Need enough data for seasonality
    ...
    def _detect_seasonality(self, df: pd.DataFrame, date_col: str, value_col: str) -> Dict[str, Any]:
        ...
        df_copy['hour'] = df_copy[date_col].dt.hour if df_copy[date_col].dt.hour.nunique() > 1 else None
    ...
    def _calculate_association(
        ...
        try:
            ...
            for val1 in contingency.index:
                for val2 in contingency.columns:
                    ...
                    if support >= min_support:
                        ...
                        lift = support / (col1_support * col2_support) if col1_support * col2_support > 0 else 0
            ...
            logger.warning(f"Error calculating association between {col1} and {col2}: {e}")
            return None

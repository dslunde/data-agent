...
class AnomalyDetector:
        ...
        try:
            ...
            for method in methods:
                if method == "iqr":
                    ...
                    outliers = self._detect_outliers_isolation_forest(
                        df, columns, contamination
                    )
                elif method == "elliptic_envelope":
                    outliers = self._detect_outliers_elliptic_envelope(
                        df, columns, contamination
                    )
                else:
    ...
    def detect_multivariate_anomalies(
        ...
        try:
            ...
            if len(features) < 2:
                return {
                    "error": "Need at least 2 features for multivariate anomaly detection"
                }
            ...
            results = {
                ...
                "anomaly_scores": {
                    ...
                    "threshold": float(
                        np.percentile(anomaly_scores, (1 - contamination) * 100)
                    ),
                },
            }
    ...
    def detect_rule_based_anomalies(
        ...
        """
        try:
            results = {"rules_applied": rules, "violations": [], "summary": {}}
    ...
    def detect_time_series_anomalies(
        ...
        try:
            ...
            if method == "statistical":
                return self._detect_ts_anomalies_statistical(
                    clean_data, date_column, value_column
                )
            elif method == "isolation_forest":
                return self._detect_ts_anomalies_isolation_forest(
                    clean_data, date_column, value_column
                )
            else:
                return {"error": f"Unknown method: {method}"}
    ...
    def _detect_outliers_isolation_forest(
        self, df: pd.DataFrame, columns: List[str], contamination: float
    ) -> Dict[str, Any]:
    ...
    def _find_consensus_outliers(
        self, method_results: Dict[str, Any], min_methods: int = 2
    ) -> List[int]:
        ...
        consensus_outliers = [
            idx for idx, count in outlier_counts.items() if count >= min_methods
        ]
        return consensus_outliers

    def _analyze_anomaly_characteristics(
        self, normal_data: pd.DataFrame, anomaly_data: pd.DataFrame, features: List[str]
    ) -> Dict[str, Any]:
    ...
    def _apply_rule(
        self, df: pd.DataFrame, rule: Dict[str, Any], rule_id: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if rule_type == "range":
                ...
                if pattern and df[column].dtype == "object":
                    violations = df[
                        ~df[column].astype(str).str.match(pattern, na=False)
                    ].index.tolist()
    ...
    def _detect_ts_anomalies_statistical(
        ...
        df_copy = df.copy()
        df_copy["rolling_mean"] = (
            df_copy[value_col].rolling(window=window_size, center=True).mean()
        )
        df_copy["rolling_std"] = (
            df_copy[value_col].rolling(window=window_size, center=True).std()
        )

        # Detect anomalies as points beyond 2 standard deviations from rolling mean
        df_copy["z_score"] = (df_copy[value_col] - df_copy["rolling_mean"]) / df_copy[
            "rolling_std"
        ]
        ...
        results = {
            ...
            "anomaly_indices": anomaly_indices,
            "anomaly_timestamps": df_copy.loc[anomaly_mask, date_col]
    ...
    def _detect_ts_anomalies_isolation_forest(
        ...
        results = {
            ...
            "anomaly_percentage": float(
                np.sum(anomaly_mask) / len(analysis_data) * 100
            ),
            "anomaly_indices": anomaly_indices,
            "anomaly_timestamps": df_copy.loc[anomaly_indices, date_col]
            .dt.strftime("%Y-%m-%d %H:%M:%S")
            .tolist(),
        }

...
class DataAgentCore:
    ...
    async def download_and_load_dataset(
        ...
        try:
            ...
            if self.verbose:
                logger.info(f"Dataset loaded: {self.dataset.shape[0]} rows, {self.dataset.shape[1]} columns")
    ...
    async def process_query(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.verbose:
                logger.info(f"Query intent: {intent.analysis_method.value} (confidence: {intent.confidence:.2f})")
    ...
    async def _execute_analysis(self, intent) -> Dict[str, Any]:
        ...
        try:
            ...
            if method == AnalysisMethod.DESCRIBE_DATASET:
                ...
                if column:
                    result = self.statistical_analyzer.count_analysis(self.dataset, column)
                else:
                ...
                if len(intent.columns) >= 2:
                    ...
                    result = self.statistical_analyzer.aggregate_data(self.dataset, group_by, agg_column)
                else:
                    result = {"error": "Need at least 2 columns for aggregation"}
            
            elif method == AnalysisMethod.FILTER_DATA:
                if intent.filters:
                    result = self.statistical_analyzer.filter_data(self.dataset, intent.filters)
                else:
                ...
                if len(intent.columns) >= 2:
                    ...
                    if len(date_cols) > 0 and len(numeric_cols) > 0:
                        ...
                        result = {"error": "Could not find suitable columns for trend analysis"}
                ...
                if len(date_cols) > 0:
                    ...
                    result = {"error": "No datetime columns found for time series analysis"}
    ...
    def _get_dataset_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "completeness_score": ((self.dataset.size - self.dataset.isnull().sum().sum()) / self.dataset.size) * 100,

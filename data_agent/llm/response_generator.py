...
class ResponseGenerator:
    ...
    async def generate_response(
        ...
        try:
            ...
            main_response = await self._generate_main_response(intent, analysis_results, evidence)
    ...
    async def _generate_error_response(
        ...
        try:
            llm_response = await self.llm_manager.generate_response(messages, temperature=0.3)
            response_text = llm_response.get("content", "I encountered an error analyzing your data.")
        except Exception as e:
    ...
    def _compile_evidence(self, intent: QueryIntent, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if intent.analysis_method == AnalysisMethod.DESCRIBE_DATASET:
            if "overview" in results:
                evidence["data_points_analyzed"] = results["overview"].get("total_rows", 0)
            ...
            if "strong_correlations" in results:
                ...
                evidence["statistical_measures"]["correlation_count"] = results.get("total_correlations", 0)
        
        elif intent.analysis_method == AnalysisMethod.OUTLIER_DETECTION:
            if "outlier_summary" in results:
                evidence["statistical_measures"]["outliers_found"] = results["outlier_summary"].get("total_outliers", 0)
                evidence["statistical_measures"]["outlier_percentage"] = results["outlier_summary"].get("outlier_percentage", 0)
        
        elif intent.analysis_method == AnalysisMethod.CLUSTERING_ANALYSIS:
            if "n_clusters" in results:
                evidence["statistical_measures"]["clusters_found"] = results["n_clusters"]
                if "metrics" in results:
                    evidence["statistical_measures"]["silhouette_score"] = results["metrics"].get("silhouette_score", 0)
    ...
    def _add_method_specific_evidence(self, intent: QueryIntent, results: Dict[str, Any], evidence: Dict[str, Any]):
        ...
        if intent.analysis_method == AnalysisMethod.COUNT_VALUES:
            if "total_count" in results:
                ...
                evidence["statistical_measures"]["unique_values"] = results.get("unique_count", 0)
        
        elif intent.analysis_method == AnalysisMethod.AGGREGATE_DATA:
            if "total_groups" in results:
                evidence["statistical_measures"]["groups_analyzed"] = results["total_groups"]
        
        elif intent.analysis_method == AnalysisMethod.TIME_SERIES_PATTERNS:
            if "date_range" in results:
                evidence["statistical_measures"]["time_span_days"] = results["date_range"].get("span_days", 0)
    
    def _generate_methodology(self, intent: QueryIntent, results: Dict[str, Any]) -> Dict[str, Any]:
        """Generate methodology explanation."""
        methodology = {
        ...
        if intent.analysis_method == AnalysisMethod.CORRELATION_ANALYSIS:
            methodology["approach"] = "Pearson correlation analysis to measure linear relationships between variables"
            ...
            methodology["approach"] = f"Unsupervised clustering using {algorithm} algorithm"
            ...
            methodology["approach"] = f"Multi-method outlier detection using {', '.join(methods)}"
            methodology["assumptions"] = [
            ...
            methodology["approach"] = f"Statistical analysis using {intent.analysis_method.value}"
            methodology["assumptions"] = ["Standard statistical assumptions apply"]
            methodology["limitations"] = ["Results depend on data quality and completeness"]
    ...
    async def _generate_main_response(
        ...
        try:
            ...
            return llm_response.get("content", "I was unable to generate a response for your analysis.")
    ...
    def _create_results_summary(self, intent: QueryIntent, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if intent.analysis_method == AnalysisMethod.DESCRIBE_DATASET:
            summary = {
                ...
                "completeness": results.get("overview", {}).get("completeness_score", 0),
            ...
            summary = {
                ...
                "most_frequent": results.get("value_distribution", {}).get("most_frequent", "N/A")
            }
            ...
            # Generic summary
            summary = {"analysis_completed": True, "method": intent.analysis_method.value}
        
        return summary
    
    def _generate_caveats(self, intent: QueryIntent, results: Dict[str, Any]) -> List[str]:
        """Generate statistical caveats and limitations."""
        caveats = []
        
        # General caveats
        if intent.confidence < 0.7:
            caveats.append("The query interpretation had moderate confidence. Results should be verified.")
        
        # Sample size caveats
        data_points = results.get("overview", {}).get("total_rows", 0)
        if data_points < 30:
            caveats.append(f"Small sample size (n={data_points}) limits the reliability of statistical conclusions.")
        ...
        if intent.analysis_method == AnalysisMethod.CORRELATION_ANALYSIS:
            ...
            if results.get("summary", {}).get("significant_count", 0) == 0:
                caveats.append("No significant correlations found - variables may be independent")
        
        elif intent.analysis_method == AnalysisMethod.CLUSTERING_ANALYSIS:
            caveats.append("Clustering results may vary with different parameters or algorithms")
            
            # Quality-based caveats
            silhouette_score = results.get("metrics", {}).get("silhouette_score", 0)
            if silhouette_score < 0.5:
                caveats.append("Clustering quality is moderate - clusters may not be well-separated")
        ...
        if "missing_data" in results:
            ...
            if missing_pct > 10:
                caveats.append(f"Dataset has {missing_pct:.1f}% missing values which may affect results")
        
        # Filter caveats
        if intent.filters:
            caveats.append(f"Results are based on filtered data ({len(intent.filters)} filter(s) applied)")

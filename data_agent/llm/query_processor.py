...
class QueryProcessor:
    ...
    async def process_query(self, query: str) -> QueryIntent:
        ...
        logger.info(f"Extracted intent: {final_intent.analysis_method.value} with confidence {final_intent.confidence}")
        
        return final_intent
    
    def _initialize_patterns(self) -> Dict[str, List[Tuple[str, QueryType, AnalysisMethod]]]:
        """Initialize regex patterns for query classification."""
        patterns = {
            'descriptive': [
                (r'\b(describe|summary|overview|basic stats?)\b', QueryType.DESCRIPTIVE, AnalysisMethod.DESCRIBE_DATASET),
                (r'\b(count|how many|number of)\b', QueryType.DESCRIPTIVE, AnalysisMethod.COUNT_VALUES),
                (r'\b(average|mean|median|sum|total|aggregate)\b', QueryType.DESCRIPTIVE, AnalysisMethod.AGGREGATE_DATA),
                (r'\b(filter|where|select|show me)\b.*\b(with|having|equals?|greater|less)\b', QueryType.DESCRIPTIVE, AnalysisMethod.FILTER_DATA),
            ],
            'exploratory': [
                (r'\b(correlat\w*|relationship|associated?)\b', QueryType.EXPLORATORY, AnalysisMethod.CORRELATION_ANALYSIS),
                (r'\b(cluster\w*|group\w*|segment\w*)\b', QueryType.EXPLORATORY, AnalysisMethod.CLUSTERING_ANALYSIS),
                (r'\b(pattern\w*|trend\w*|distribution)\b', QueryType.EXPLORATORY, AnalysisMethod.PATTERN_RECOGNITION),
            ],
            'diagnostic': [
                (r'\b(outlier\w*|anomal\w*|unusual|strange)\b', QueryType.DIAGNOSTIC, AnalysisMethod.OUTLIER_DETECTION),
                (r'\b(quality|issue\w*|problem\w*|missing|incomplete)\b', QueryType.DIAGNOSTIC, AnalysisMethod.DATA_QUALITY_CHECK),
            ],
            'temporal': [
                (r'\b(over time|time series|temporal|trend\w*)\b', QueryType.TEMPORAL, AnalysisMethod.TREND_ANALYSIS),
                (r'\b(seasonal\w*|cyclic\w*|periodic)\b', QueryType.TEMPORAL, AnalysisMethod.SEASONALITY_DETECTION),
            ],
            'comparative': [
                (r'\b(compare|comparison|difference|vs|versus)\b', QueryType.COMPARATIVE, AnalysisMethod.GROUP_COMPARISON),
                (r'\b(segment\w*|group by|breakdown)\b', QueryType.COMPARATIVE, AnalysisMethod.SEGMENT_ANALYSIS),
            ],
            'causal': [
                (r'\b(cause\w*|reason\w*|why|explain\w*|impact|effect)\b', QueryType.CAUSAL, AnalysisMethod.PATTERN_RECOGNITION),
            ]
        }
    ...
    async def _extract_intent_with_llm(self, query: str, initial_intent: Dict[str, Any]) -> Dict[str, Any]:
...
Set confidence between 0.1 and 1.0 based on how clear the intent is."""
    ...
    def _fallback_intent_extraction(self, query: str, initial_intent: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _validate_and_refine_intent(self, intent_data: Dict[str, Any], query: str) -> QueryIntent:
        ...
        if self.available_columns:
            validated_columns = [col for col in columns if col in self.available_columns]
        ...
        for col in columns:
            ...
            for available_col in self.available_columns:
                ...
                if col_lower in available_lower or available_lower in col_lower:
                    score = len(col_lower) / len(available_lower) if available_lower else 0
                    if score > best_score:
                        best_score = score

...
class TestStatisticalAnalyzer:
    ...
    def test_aggregate_data(self, analyzer, sample_df):
        ...
        result = analyzer.aggregate_data(sample_df, 'category', 'value', ['count', 'mean', 'std'])
...
class TestPatternAnalyzer:
    ...
    def test_correlation_analysis(self, analyzer, corr_df):
        ...
        xy_found = any(
            (pair['column1'] == 'x' and pair['column2'] == 'y') or 
    ...
    def test_clustering_analysis_kmeans(self, analyzer, corr_df):
        ...
        features = ['x', 'y', 'z']
        result = analyzer.clustering_analysis(corr_df, features, algorithm='kmeans', n_clusters=2)
    ...
    def test_time_series_patterns(self, analyzer):
        ...
        values = np.sin(np.arange(100) * 2 * np.pi / 30) + np.random.normal(0, 0.1, 100)  # Monthly pattern
    ...
    def test_invalid_clustering_parameters(self, analyzer, corr_df):
        ...
        result = analyzer.clustering_analysis(corr_df, ['x', 'y'], algorithm='invalid_algo')
        assert 'error' in result
...
class TestAnomalyDetector:
    ...
    def test_detect_outliers_iqr(self, detector, outlier_df):
        ...
        assert 'iqr' in result.get('methods_used', result.get('methods', result.get('method', [])))
    ...
    def test_detect_outliers_zscore(self, detector, outlier_df):
        ...
        assert 'zscore' in result.get('methods_used', result.get('methods', result.get('method', [])))
    ...
    def test_outlier_detection_multiple_columns(self, detector, outlier_df):
        ...
        outlier_df.loc[95:99, 'second_value'] = [500, -200, 600, -300, 700]  # Add outliers
        
        result = detector.detect_outliers(outlier_df, ['value', 'second_value'], methods=['iqr'])
    ...
    def test_invalid_method_handling(self, detector, outlier_df):
        ...
        result = detector.detect_outliers(outlier_df, ['value'], methods=['invalid_method'])
        assert 'error' in result
    
    def test_invalid_column_handling(self, detector, outlier_df):
        """Test handling of invalid column names."""
        result = detector.detect_outliers(outlier_df, ['nonexistent_column'], methods=['iqr'])
        assert 'error' in result

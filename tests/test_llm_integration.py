...
from data_agent.llm.query_processor import (
    QueryProcessor,
    QueryIntent,
    QueryType,
    AnalysisMethod,
...
class TestLLMManager:
    ...
    def test_llm_manager_openai_only(self, mock_openai):
        ...
        assert manager.current_client is not None
        assert (
            manager.get_current_provider() == "anthropic"
        )  # Should prefer anthropic when auto
    ...
    def test_llm_manager_provider_switching(self):
        ...
        manager.clients = {"openai": openai_client, "anthropic": anthropic_client}

        # Test switching
        assert manager.switch_provider("openai")
        assert manager.current_client == openai_client

        assert manager.switch_provider("anthropic")
        assert manager.current_client == anthropic_client

        # Test switching to unavailable provider
        assert not manager.switch_provider("nonexistent")
    ...
    async def test_llm_manager_generate_response(self):
        ...
        primary_client.generate_response = AsyncMock(
            return_value={"error": "Primary failed"}
        )
        fallback_client.generate_response = AsyncMock(
            return_value={"content": "Fallback success"}
        )
    ...
    def test_filter_extraction(self, mock_llm_manager):
        ...
        test_cases = [
            ...
            (
                ...
                {},
            ),  # Complex date filters might not be caught
        ]
...
class TestResponseGenerator:
    ...
    def sample_analysis_results(self):
        ...
        return {
            "overview": {"total_rows": 100, "total_columns": 5, "memory_usage_mb": 0.5},
            "numeric_summary": {
                "value": {"mean": 50.0, "std": 15.0, "min": 10.0, "max": 90.0}
            },
            "categorical_summary": {
                "category": {"unique_count": 3, "most_common": "A"}
            },
        }
    ...
    @pytest.mark.asyncio
    async def test_generate_response_descriptive(
        self,
        mock_llm_manager,
        sample_intent,
        ...
        result = await generator.generate_response(
            sample_intent, sample_analysis_results, sample_dataset_info
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_response_with_error(
        self, mock_llm_manager, sample_intent, sample_dataset_info
    ):
        ...
        result = await generator.generate_response(
            sample_intent, error_results, sample_dataset_info
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_response_llm_failure(
        self,
        mock_llm_manager,
        sample_intent,
        sample_analysis_results,

...
class TestCLIArguments:
    ...
    def test_cli_argument_parsing(self, mock_run_data_agent):
        ...
        assert call_kwargs['cache_enabled'] == True
...
class TestCLIDataLoading:
    ...
    async def test_run_data_agent_download_mode(self):
        ...
        with patch('data_agent.cli.DataAgentCore') as mock_core_class:
            mock_core = Mock()
            mock_core.download_and_load_dataset = AsyncMock(return_value={'shape': (100, 5)})
            mock_core_class.return_value = mock_core
...
class TestCLIInteractiveMode:
    ...
    async def test_interactive_query_processing(self):
        ...
        with patch('data_agent.cli.DataAgentCore') as mock_core_class:
            mock_core = Mock()
            mock_core.download_and_load_dataset = AsyncMock(return_value={
                'shape': (100, 5),
            ...
            with patch('data_agent.cli.test_llm_connectivity') as mock_connectivity:
                ...
                with patch('click.prompt', side_effect=['test query', 'exit']):
                    with patch('click.echo') as mock_echo:
                        ...
                        response_calls = [call for call in mock_echo.call_args_list 
...
class TestCLIErrorHandling:
    ...
    def test_general_exception_handling(self):
        ...
        with patch('data_agent.cli.run_data_agent', side_effect=Exception('Test error')):
            result = runner.invoke(main, [])

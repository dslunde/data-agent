...
from data_agent.data.downloader import get_default_downloader
from data_agent.data.loader import get_default_loader
from data_agent.llm.query_processor import QueryType, AnalysisMethod
...
class TestAnalysisIntegration:
    ...
    def test_correlation_analysis_integration(self, analysis_dataset):
        ...
        xy_found = any(
            (pair['column1'] == 'feature_x' and pair['column2'] == 'feature_y') or
            (pair['column1'] == 'feature_y' and pair['column2'] == 'feature_x')
    ...
    def test_clustering_integration(self, analysis_dataset):
        ...
        result = analyzer.clustering_analysis(analysis_dataset, features, algorithm='kmeans', n_clusters=3)
    ...
    def test_outlier_detection_integration(self, analysis_dataset):
        ...
        # Test IQR method
        result = detector.detect_outliers(analysis_dataset, columns=['feature_y'], method='iqr')
...
class TestEndToEndIntegration:
            ...
            try:
                ...
                with patch('data_agent.core.create_llm_manager') as mock_llm_factory:
                    ...
                    mock_manager.generate_response.side_effect = [
                        ...
                        mock_llm_responses['response_generation']  # Response generation
                    ]
    ...
    async def test_core_integration_with_errors(self, mock_llm_responses):
        ...
        with patch('data_agent.core.create_llm_manager') as mock_llm_factory:
            ...
            mock_manager.generate_response.return_value = {'error': 'LLM service unavailable'}
            mock_llm_factory.return_value = mock_manager
